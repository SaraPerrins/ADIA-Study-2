...
dat$y <-dat$DEPBTOT16 #dat$YMDSYMP
summary(dat$y)
dat <- dat[!is.na(dat$y), ] #n=807
#===============================================================================
...
prp(ptree_causal, type = 4, # left and right split labels (see Figure 2)
    ...
    main = "Physical health causal tree", # main title
...
dat$node.cau  <- factor(predict(as.party(ptree_causal),
...
fit0   <- svyglm(y ~ anyACE
                 , design = sdw, family = gaussian)
cbind(coef(fit0), confint(fit0))[2,]

fit1 <- svyglm(y ~ anyACE:node.cau + node.cau  - 1,
               design = sdw, family = gaussian)
