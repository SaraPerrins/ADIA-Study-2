...
summary(dat$cw)
dat$cw <- NA
dat$cw[dat$anyACE == 1] <- dat$w * sum(dat$anyACE)
dat$cw[dat$anyACE == 0] <- dat$w * sum(!dat$anyACE)
...
rpart.plot(ptree_causal, roundint = FALSE) #Ye : chose the plot you like to save below
