...
dat$y <- dat$future_op_pos #exposed and no missing on outcome 0=620 1=27 na=324
table(dat$y,useNA='ifany')
summary(dat$y)
hist(dat$y)
dat <- dat[!is.na(dat$y), ] #n=581

#dicotomized y to binary outcome for case-control matching,use y0 for matching purpose only
dat$y0[dat$y<4.5]      <- 0
dat$y0[dat$y>=4.5]     <- 1
table(dat$y0)
...
tree <- with(df0, rpart(y ~ . ,
                                    data = data.frame(y, Z), cp = 0, xval = 5,
                                    method = "anova"
...
#1 0.07511      0     1.000  1.010 0.151
#2 0.03015      1     0.925  0.955 0.151
#3 0.01712      2     0.895  1.008 0.149
#4 0.00117      3     0.878  0.979 0.138
#5 0.00000      4     0.876  0.983 0.138
...
#. 1SE rule
#  1.01+1*0.156=1.17
#  tree$cptable[, "CP"][which.min(tree$cptable[, "xerror"])]=0.0117
...
prp(ptree, type = 4, # left and right split labels (see Figure 2)
    ...
    main = "Positive future aspiration classical tree", # main title
...
cfite <- glm(y ~ node.rtree + strata(id), data=df1, family="gaussian")
summary(cfite)
anova(cfite, update(cfite, . ~ 1))

cfite0 <- glm(y ~ node.rtree, data=df1, family="gaussian")
summary(cfite0)
