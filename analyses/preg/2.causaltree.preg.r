...
out <- "RB_preg"
dat$y <- dat$RB_preg #0=583 1=227 na=554
table(dat$y, useNA = "ifany")
dat <- dat[!is.na(dat$y), ] #n=810
#===============================================================================
...
dat$anyACE[is.na(dat$anyACE_T)] <- 0
...
#Ye-no split, fix opcp to 0.01
#CP   nsplit    rel     error xerror  xstd
#1 0.0262445      0     1.000  1.000 0.116
#2 0.0195684      1     0.974  1.007 0.117
#3 0.0000246      2     0.954  0.976 0.111
#4 0.0000000      3     0.954  0.976 0.111

#opcp <- tree$cptable[, 1][which.min(tree$cptable[, 4])]
opcp <- 0.01 #nosplit, fix opcp to 0.01, get 3 nodes
...
prp(ptree_causal, type = 4, # left and right split labels (see Figure 2)
    ...
    main = "Preganacy causal tree", # main title
