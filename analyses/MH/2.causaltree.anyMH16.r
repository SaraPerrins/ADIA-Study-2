...
dat$cohort[as.numeric(substr(dat$ChildDOB,6,9))==1994] <- 9
dat$cohort[as.numeric(substr(dat$ChildDOB,6,9))==1995] <- 10
...
prp(ptree_causal, type = 4, # left and right split labels (see Figure 2)
    ...
    main = "Violent behavior causal tree", # main title
...
df1$ACE <- factor(df1$anyACE
                  , levels = c(FALSE, TRUE),  labels = c("NoACE", "ACE"))

df1$ace_pf <- paste(df1$ACE, df1$node.cau, sep = "_") # there may be an NA group

df1$ace_pf[is.na(df1$node.cau)] <- NA

df1$ace_pf <- factor(df1$ace_pf)
is.numeric(df1$ace_pf)
...
left_join(tb_r, tb_w, by = "var", suffix = c(".raw", ".weighted")) %>%
  ...
  gtsave(paste0("output/raw_weighted.html"))
...
anova(fit0, fit1)
table(df1$ace_pf[!is.na(df1$y)]) #group size, no missing on y


###6.- Doubly robust test
#' BUT listwise deletion !!
fit0dr   <- svyglm(as.numeric(y) ~ anyACE
                   + center + childrace_bsl + cohort
                   + childsex_bsl + caregiver_married16 + hh_income16
                   , design = sdw, family = gaussian)
cbind(coef(fit0dr), confint(fit0dr))[2,]

fit1dr <- svyglm(as.numeric(y) ~ anyACE:node.cau + node.cau  - 1
                 + center + childrace_bsl + cohort
                 + childsex_bsl + caregiver_married16 + hh_income16
                 , design = sdw, family = gaussian)
