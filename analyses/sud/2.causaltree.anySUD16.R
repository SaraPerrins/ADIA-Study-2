...
file_name <- paste0("data/", outv, "LS.w.Rds")
...
#===============================================================================
dat <- readRDS(paste0("data/", outv, "LS.w.Rds"))
...
###Causal Trees (CT)
tree_causal <- with(df0, causalTree(factor(y) ~ .,
                             data = cbind(y, PF)
                             , treatment = anyACE
                             , weights = cw
                             , split.Rule = "CT"
...
#fit0   <- svyglm(as.numeric(y) ~ anyACE, design = sdw, family = gaussian)
fit0   <- svyglm(factor(y) ~ anyACE
                 , design = sdw, family = quasibinomial(link="logit"))
cbind(coef(fit0), confint(fit0))[2,]

#fit1 <- svyglm(as.numeric(y) ~ anyACE:node.cau + node.cau  - 1,design = sdw, family = gaussian)
